...
export function BindingPanel() {
  const { scene, selectedId, varManager, bindingEngine, updateShape, renderScene } = useEditorStore();
  const shape = selectedId ? scene.get(selectedId) : null;

  const [editingBindingIdx, setEditingBindingIdx] = useState<number | null>(null);
  ...
  const updateMapping = (idx: number, upd: Partial<ValueMapping>) => {
    if (!selectedId || !shape) return;
    const bindings = [...shape.bindings];
    bindings[idx] = { ...bindings[idx], mapping: { ...bindings[idx].mapping, ...upd } as ValueMapping };
    updateShape(selectedId, { bindings });
    bindingEngine?.reindexShape(selectedId);
  };

  // 获取当前选中变量的定义
  const getVarDef = (vid: string): VariableDef | undefined => allVars.find((v) => v.id === vid);
  ...
  return (
    <div className="panel">
      ...
      <button className="btn btn-primary btn-full" onClick={addBinding} disabled={allVars.length === 0}>
        + 添加绑定
      </button>
      {allVars.length === 0 && <div className="panel-hint">请先在"点表管理"中添加变量</div>}
      ...
      {shape.bindings.map((binding, idx) => {
        ...
        return (
          <div key={idx} className="binding-item">
            <div className="binding-header" onClick={() => setEditingBindingIdx(isEditing ? null : idx)}>
              <div className="binding-summary">
                <span className={"var-type-badge " + binding.variableType.toLowerCase()}>
                  {binding.variableType}
                </span>
                <span className="binding-var-id">{binding.variableId || "(未选择)"}</span>
              </div>
              <div className="binding-arrows">
                <span className="binding-arrow">→</span>
                <span className="binding-prop">{binding.targetProp}</span>
              </div>
              <button className="btn-icon" onClick={(e) => { e.stopPropagation(); removeBinding(idx); }}>✕</button>
            </div>
              ...
              <div className="binding-detail">
                ...
                <div className="prop-group">
                  ...
                  <select
                    ...
                    onChange={(e) => updateBinding(idx, { targetProp: e.target.value })}
                  >
                ...
                <div className="prop-group">
                  ...
                  <select
                    ...
                    onChange={(e) => updateMapping(idx, { type: e.target.value as ValueMapping["type"] })}
                  >
                  ...
                  <div className="binding-mapping-config">
                    <div className="prop-group">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const map = { ...(binding.mapping as any).map, "0": e.target.value };
                    ...
                    <div className="prop-group">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          const map = { ...(binding.mapping as any).map, "1": e.target.value };
                          updateMapping(idx, { map });
                        }}
                        placeholder="#00FF00"
                      />
                ...
                {binding.mapping.type === "range" && (
                  <div className="binding-mapping-config">
                    <div className="prop-group">
                      <label>输入范围</label>
                      <input
                        type="number" style={{ width: "45%" }}
                        value={(binding.mapping as any).from?.[0] ?? 0}
                        onChange={(e) => updateMapping(idx, {
                          from: [Number(e.target.value), (binding.mapping as any).from?.[1] ?? 100] as [number, number],
                        })}
                      />
                      <span>~</span>
                      <input
                        ...
                        onChange={(e) => updateMapping(idx, {
                          from: [(binding.mapping as any).from?.[0] ?? 0, Number(e.target.value)] as [number, number],
                        })}
                      />
                    </div>
                    <div className="prop-group">
                      ...
                      <input
                        ...
                        onChange={(e) => updateMapping(idx, {
                          to: [Number(e.target.value), (binding.mapping as any).to?.[1] ?? 270] as [number, number],
                        })}
                      />
                      <span>~</span>
                      <input
                        ...
                        onChange={(e) => updateMapping(idx, {
                          to: [(binding.mapping as any).to?.[0] ?? 0, Number(e.target.value)] as [number, number],
                        })}
                      />
                    </div>
                  </div>
                ...
                <div className="binding-test">
                  ...
                  {vDef && (vDef.type === "DI" || vDef.type === "DO") ? (
                    <button
                      ...
                      onClick={() => {
                        const current = varManager?.getValue(binding.variableId)?.value;
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          varManager?.setValue(binding.variableId, Number((e.target as HTMLInputElement).value));
                        }
                      }}
                    />
                  )}
