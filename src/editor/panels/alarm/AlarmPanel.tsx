...
export function AlarmPanel() {
  const { alarmManager, acknowledgeAlarm, acknowledgeAllAlarms } = useEditorStore();
  const [activeTab, setActiveTab] = useState<"active" | "history" | "soe">("active");
  ...
  return (
    <div className="panel alarm-panel">
      <div className="panel-title">
        报警事件
        <span className="panel-badge" style={{ background: unackCount > 0 ? "var(--danger)" : "var(--success)" }}>
          {unackCount > 0 ? unackCount + " 未确认" : "正常"}
        </span>
      </div>

      <div className="alarm-tabs">
        <button className={"alarm-tab" + (activeTab === "active" ? " active" : "")} onClick={() => setActiveTab("active")}>
          活跃 ({alarmCount})
        </button>
        <button className={"alarm-tab" + (activeTab === "history" ? " active" : "")} onClick={() => setActiveTab("history")}>
          历史
        </button>
        <button className={"alarm-tab" + (activeTab === "soe" ? " active" : "")} onClick={() => setActiveTab("soe")}>
          SOE
        </button>
      </div>
      ...
      {activeTab === "active" && (
        <div className="alarm-content">
          {alarmCount > 0 && (
            <button className="btn btn-sm btn-full" onClick={acknowledgeAllAlarms} style={{ marginBottom: 4 }}>
              确认全部 ({unackCount})
            </button>
          )}
          {activeAlarms.length === 0 && <div className="panel-hint">无活跃报警</div>}
          {activeAlarms.map((alarm) => {
            const sc = SEVERITY_CONFIG[alarm.severity] ?? SEVERITY_CONFIG.warning!;
            return (
              <div key={alarm.id} className="alarm-row" style={{ borderLeftColor: sc.color }}>
                <div className="alarm-row-header">
                  <span className="alarm-severity" style={{ background: sc.color }}>{sc.label}</span>
                ...
                <div className="alarm-row-detail">
                  {alarm.message} · 当前值: {String(alarm.value)} / 阈值: {alarm.threshold}
                </div>
                <div className="alarm-row-footer">
                  <span className="alarm-status" style={{
                    color: alarm.status === "active" ? sc.color : "var(--text-secondary)"
                  }}>
                    {alarm.status === "active" ? "未确认" : "已确认"}
                  </span>
                  {alarm.status === "active" && (
                    <button className="btn btn-sm" onClick={() => acknowledgeAlarm(alarm.id)}>
                      确认
                    </button>
                  )}
                </div>
          ...
          <div className="soe-list">
            {soeRecords.map((soe) => (
              <div key={soe.id} className="soe-row">
                ...
                <span className={"soe-quality " + soe.quality}>{soe.quality}</span>
              </div>
            ))}
          </div>
        </div>
