...
export function ReportPanel() {
  ...
  return (
    <div className="panel report-panel">
        ...
        >
          ...
          {configs.map((c) => (
            <option key={c.id} value={c.id}>
              {c.name} ({c.variableIds.length} 变量)
            </option>
          ))}
        </select>
      </div>
      ...
      {configs.length === 0 && (
        <div className="panel-hint">暂无报表配置，点击"预设"添加</div>
      )}
      ...
      {configs.length > 0 && (
        <div className="report-list">
          <div
            ...
            style={{ fontSize: 12, border: "none", marginTop: 8 }}
          >
            已有配置
          </div>
          {configs.map((c) => (
            <div
              key={c.id}
              className="report-config-item"
              onClick={() => setSelectedCfg(c.id)}
            >
