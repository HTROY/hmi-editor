...
use hmi_io_db::repo::{
    AlarmOccurrenceRow, AlarmStreamEventRow, Repo, SoeRow,
};
use std::sync::Arc;
...
pub fn spawn_persister(
    ...
    tokio::spawn(async move {
        while let Some(ev) = rx.recv().await {
            match ev {
                OutEvent::Occurrence { occurrence, mut event } => {
                    let occ_row: AlarmOccurrenceRow = (&occurrence).into();
                    let mut ev_row: AlarmStreamEventRow = (&event).into();
                    if let Err(e) = repo.upsert_alarm_occurrence(&occ_row) {
                        log::error!("Persist alarm occurrence failed: {}", e);
                    }
                    match repo.insert_alarm_stream_event(&mut ev_row) {
...
mod tests {
    ...
    async fn persister_writes_db_and_broadcasts() {
        ...
        let eng = AlarmEngine::with_soe_seq(tx, repo.max_soe_seq());
        let rule: AlarmRule = repo.list_alarm_rules().unwrap()[0].clone().into();
        eng.load_rules(vec![rule]);

        eng.on_point(&PointValue::new("P1", 120.0, "good", 1000));
        tokio::time::sleep(std::time::Duration::from_millis(80)).await;

        let occ_rows = repo.list_active_alarm_occurrences().unwrap();
        assert_eq!(occ_rows.len(), 1);
        let (_, soe_rows) = repo.query_soe(None, None, None, None, 1, 10).unwrap();
        ...
        let evs = repo.query_occurrence_stream_events(&occ_id).unwrap();
        assert_eq!(evs.len(), 2);
        ...
        assert!(repo.list_recovered_unacked().unwrap().is_empty());
    }
