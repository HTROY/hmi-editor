...
pub async fn ensure_admin_seeded(repo: &Repo) -> anyhow::Result<Option<String>> {
    ...
    repo.create_user("admin", &hash, "admin", true).await?;
    Ok(Some(password))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn ensure_admin_seeded_creates_admin_with_random_password_once() {
        let repo = Repo::new(":memory:").await.unwrap();
        let password = ensure_admin_seeded(&repo).await.unwrap().unwrap();
        ...
        assert!(ensure_admin_seeded(&repo).await.unwrap().is_none());
