...
#[derive(Debug,Clone,Serialize,Deserialize)] struct PluginConfig { host:String,port:u16,slave_id:u8,#[serde(default)] points:Vec<Pc> }
#[derive(Debug,Clone,Serialize,Deserialize)] struct PluginState { host:String,port:u16,slave_id:u8,connected:bool,scan_count:u64,socket:i32,points:Vec<Pc> }

fn sv(s:&PluginState)->FnResult<()>{var::set("state",&serde_json::to_string(s)?)?;Ok(())}
fn ld() -> FnResult<Option<PluginState>> {
    let json: Option<String> = var::get("state")?;
    Ok(json.and_then(|j| serde_json::from_str(&j).ok()))
}

#[plugin_fn] pub fn plugin_init(Json(mut c):Json<PluginConfig>)->FnResult<i32>{
    lm(2,&format!("Modbus TCP init: {}:{}, slave={}, {} pts",c.host,c.port,c.slave_id,c.points.len()));
    sv(&PluginState{host:c.host,port:c.port,slave_id:c.slave_id,connected:false,scan_count:0,socket:-1,points:std::mem::take(&mut c.points)})?;Ok(0)
}
#[plugin_fn] pub fn plugin_connect()->FnResult<i32>{let mut s=ld()?.expect("ni");
    lm(2,&format!("Modbus TCP connecting {}:{}...",s.host,s.port));
    let sk=tc(&s.host,s.port as i32);if sk<0{lm(1,"connect failed");s.connected=false;s.socket=-1;sv(&s)?;return Ok(1);}
    s.connected=true;s.socket=sk;lm(2,&format!("connected fd={}",sk));sv(&s)?;Ok(0)
}
...
#[plugin_fn] pub fn plugin_scan_points()->FnResult<i32>{
    let mut s=ld()?.expect("ni");
    if!s.connected||s.socket<0{return Ok(1);}
    s.scan_count+=1;
    let now=unsafe{host_now_ms()};
    for pt in &s.points.clone(){
        match mb_read(&s, pt){
            Ok(v)=>{rp(&pt.variable_id,v,"good",now);}
            Err(e)=>{rp(&pt.variable_id,0.0,"bad",now);lm(1,&e);}
        }
    }
    sv(&s)?;Ok(0)
}
...
fn mb_read(state: &PluginState, pt: &Pc) -> Result<f64, String> {
    let (fc, addr): (u8, u16) = if pt.address.starts_with("coil:") {
        (0x01, pt.address[5..].parse::<u16>().map_err(|e| format!("bad addr: {}", e))?)
    } else if pt.address.starts_with("holding_register:") {
        (0x03, pt.address[17..].parse::<u16>().map_err(|e| format!("bad addr: {}", e))?)
    } else if pt.address.starts_with("input_register:") {
        (0x04, pt.address[15..].parse::<u16>().map_err(|e| format!("bad addr: {}", e))?)
    } else if pt.address.starts_with("discrete_input:") {
        (0x02, pt.address[14..].parse::<u16>().map_err(|e| format!("bad addr: {}", e))?)
    } else {
        return Err(format!("unknown addr type: {}", pt.address));
    };

    let mut frame = vec![0u8; 12];
    frame[0]=((state.scan_count>>8)&0xff) as u8; frame[1]=(state.scan_count&0xff) as u8;
    frame[2]=0;frame[3]=0;
    frame[4]=0;frame[5]=6;
    frame[6]=state.slave_id;
    frame[7]=fc;
    frame[8]=((addr>>8)&0xff) as u8; frame[9]=(addr&0xff) as u8;
    frame[10]=0;frame[11]=1;

    let hex_dump: String = frame.iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ");
    let summary = format!("{} addr={} count=1", if fc==0x01 {"RD_COIL"} else if fc==0x03 {"RD_HREG"} else if fc==0x02 {"RD_DIN"} else {"RD_IREG"}, addr);
    rpt("tx","modbus",&hex_dump,&summary);

    let sent = ts(state.socket, &frame);
    if sent < 0 { return Err("send failed".into()); }

    let data = tr(state.socket, 2000);
    if data.is_empty() { return Err("recv timeout".into()); }
    if data.len() < 9 { return Err(format!("short resp: {} bytes", data.len())); }

    let rx_hex: String = data.iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ");
    rpt("rx","modbus",&rx_hex,"response");

    if data[7] != fc { return Err(format!("bad fc: {}", data[7])); }

    let raw: f64 = match fc {
        0x01|0x02 => {
            if data.len() < 10 || data[8] < 1 { return Err("empty coil data".into()); }
            ((data[9] & 0x01) as f64)
        }
        0x03|0x04 => {
            if data.len() < 10 || data[8] < 2 { return Err("empty register data".into()); }
...
#[plugin_fn] pub fn plugin_write_point(Json(i):Json<Wi>)->FnResult<i32>{
    let s=ld()?.expect("ni");
    if!s.connected||s.socket<0{return Ok(2);}
    let pt=s.points.iter().find(|p|p.variable_id==i.name).cloned();
    if pt.is_none(){return Ok(3);}
    let pt=pt.unwrap();
    let (fc, addr): (u8, u16) = if pt.address.starts_with("coil:") {
        (0x05, pt.address[5..].parse::<u16>().map_err(|_| "bad addr".to_string()).unwrap_or(0))
    } else if pt.address.starts_with("holding_register:") {
        (0x06, pt.address[17..].parse::<u16>().map_err(|_| "bad addr".to_string()).unwrap_or(0))
    } else { return Ok(3); };

    let mut frame = vec![0u8; 12];
    frame[0]=((s.scan_count>>8)&0xff) as u8; frame[1]=(s.scan_count&0xff) as u8;
    frame[2]=0;frame[3]=0;
    frame[4]=0;frame[5]=6;
    frame[6]=s.slave_id;
    frame[7]=fc;
    ...
    frame[10]=((v>>8)&0xff) as u8; frame[11]=(v&0xff) as u8;

    let hex_dump: String = frame.iter().map(|b| format!("{:02x}", b)).collect::<Vec<_>>().join(" ");
    rpt("tx","modbus",&hex_dump,&format!("WRITE {}={}", pt.address, i.value));
    let sent = ts(s.socket, &frame);
    if sent < 0 { return Ok(4); }

    let data = tr(s.socket, 2000);
    if data.len() < 9 { return Ok(5); }
    lm(2,&format!("write {}={} done", i.name, i.value));
    Ok(0)
}
#[plugin_fn] pub fn plugin_get_name()->FnResult<String>{Ok("Modbus TCP".to_string())}
