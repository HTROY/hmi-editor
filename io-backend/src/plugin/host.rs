...
pub struct NetworkState {
    next_id: i32,
    tcp: HashMap<i32, TcpStream>,
    udp: HashMap<i32, UdpSocket>,
}
impl NetworkState {
    fn alloc_tcp(&mut self, s: TcpStream) -> i32 {
        let id = self.next_id;
        self.next_id += 1;
        self.tcp.insert(id, s);
        id
    }
    fn alloc_udp(&mut self, s: UdpSocket) -> i32 {
        let id = self.next_id;
        self.next_id += 1;
...
impl PluginHost {
    ...
    pub fn load_plugin(
        ...
        let plugin = Plugin::new(&manifest, functions, true).map_err(|e| {
            anyhow::anyhow!("Failed to create Extism plugin '{}': {}", plugin_name, e)
        })?;
...
fn extract_hex(msg: &str) -> &str {
    if let Some(p) = msg.find(": ") {
        let r = &msg[p + 2..];
        if r.chars().all(|c| c.is_ascii_hexdigit() || c == ' ') && r.len() > 2 {
            return r;
        }
    }
    msg
            ...
            Function::new(
                ...
                |p, i, _o, u| {
                    ...
                    if msg.contains("TX:") || msg.contains("tx:") {
                        inner
                            .monitor
                            .log_packet(&pn, "tx", &pn, extract_hex(&msg), &msg);
                    } else if msg.contains("RX:") || msg.contains("rx:") {
                        inner
                            .monitor
                            .log_packet(&pn, "rx", &pn, extract_hex(&msg), &msg);
                    }
                    Ok(())
                },
            )
            ...
            Function::new(
                ...
                |p, i, _o, u| {
                    ...
                    if !hex.is_empty() {
                        ...
                        inner
                            .monitor
                            .log_packet(&inner.plugin_name, &dir, &proto, &hex, &sum);
                    }
                    Ok(())
                },
            )
                    ...
                    let r: i64 = match net.tcp.get_mut(&sid) {
                        Some(s) => {
                            ...
                            match s.read(&mut buf) {
                                Ok(0) => -2i64,
                                Ok(n) => match p.memory_alloc(n as u64) {
                                    Ok(handle) => {
                    ...
                    let r: i64 = match net.udp.get_mut(&sid) {
                        Some(s) => {
                            ...
                            match s.recv_from(&mut buf) {
                                Ok((n, _)) => match p.memory_alloc(n as u64) {
                                    Ok(handle) => {
                                        if let Ok(dst) = p.memory_bytes_mut(handle) {
                                            dst[..n].copy_from_slice(&buf[..n]);
                                        }
