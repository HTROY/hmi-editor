...
fn main() -> anyhow::Result<()> {
    ...
    rt.block_on(async move {
        ...
        for (n, s) in &statuses { log::info!("  Plugin '{}': {}", n, s); }
        ...
        log::info!("  Monitor API:  http://localhost:{}/api/monitor/overview", web_port);
        ...
        match repo.insert_plugin(&inst.name, &inst.wasm_file, &cj) {
            Ok(pid) => {
                for pt in &inst.points {
                    let _ = repo.insert_point(pid, &pt.id, &pt.address, &pt.data_type, &pt.byte_order, pt.scale, pt.offset, &pt.var_type, "");
                }
