...
>
> 进度：F21 冒烟回归（后端+插件+WS 推送）、F12（AlarmManager 仓储化 + 共享夹具）、F18（plugin-kit）
> 已落地（2026-08）。剩余：**F01** 全 API + WS 认证与 RBAC（P0，最难，按“执行策略 3”拆四步）。
...
| 7    | **F21**（部分）E2E 测试基建（test-servers + 临时 SQLite）             | P3     | ★★★ 偏难    | ☑ 已完成（冒烟版） |
| 8    | **F12** AlarmManager 仓储化拆分（前后端共享夹具）                     | P2     | ★★★ 难      | ☑ 已完成           |
| 9    | **F18** 提取 plugin-kit（三个 wasm guest 去重）                       | P3     | ★★★ 难      | ☑ 已完成           |
