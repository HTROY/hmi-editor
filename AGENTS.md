...
- `npm run test:e2e` (or `node e2e/smoke-test.mjs`) — E2E smoke regression（F21）：用临时 SQLite + 真实后端 + iec104/opcua 测试服务器 + 插件，断言监控连接状态、WS 推送点位质量与报警规则（详见 `e2e/README.md`）；CI 的 `e2e` job 并行跑同一脚本
...
- Guests get no host network API: they use `std::net::TcpStream` directly (host_tcp_* APIs from the old Extism era no longer exist). Shared guest scaffolding lives in `crates/shared/plugin-kit` (F18): `PointCfg`（原各 guest 重复的 `Pc`）、`Kit`（STATE/STREAM 容器 + `take_stream`/`put_stream` 跨 await 不持锁 + `begin_scan`/`status`/`link_lost`）、`PluginEvents` trait 与 `report_failure` 错误策略；`modbus-tcp`（拆为 `codec.rs`/`transport.rs`/`state.rs`）、`opc-ua`、`iec104` 的 `lib.rs` 只保留协议握手/读写与宿主生命周期骨架。`opc-ua` 和 `iec104` 复用共享 codec（`ua-core` all-little-endian, 1601-epoch DateTime, `iec104-core` ASDU/frame codecs）；`modbus-tcp` 自持 MBAP 编解码。
